...
import { setIbkrGatewayBridge } from "gloom-ibkr/gateway-bridge";
import { refreshGatewayData } from "./gateway/helpers";
import { buildPersistedIbkrGatewayConfig } from "gloom-ibkr/config";
...
export const ibkrGatewayPlugin: GloomPlugin = {
  ...
  // The terminal is one. The desktop is one too: its Bun process loads this
  // plugin through `index.tsx` and owns the socket, while the view loads
  // `index.browser.tsx` and reaches it through the host's remote broker
  // client. The hosted web app has no Bun process, so it is left out.
